...
function getClient() {
    ...
    $client->setAuthConfig('/app/credentials_bernardmosella.json');
    $client->setAccessType('offline');
    $client->setPrompt('select_account consent');

    // Load previously authorized token from a file, if it exists.
    $tokenPath = '/app/barry_token.json';
...
if (1) {
    ...
    //$calendarId = 'ap15ihlbl5rrvt86bsmp4pd6gk@group.calendar.google.com';
    $calendarId = 'aat1o6doquvnh7iqg1p5n41pcg@group.calendar.google.com';
    $optParams = array(
      'maxResults' => 2,
      'orderBy' => 'startTime',
      'singleEvents' => true,
    ...
    if (empty($events)) {
        ...
        foreach ($events as $event) {
            ...
            printf("[%s] %s (%s)\n", $event->id,$event->getSummary(), $start);
